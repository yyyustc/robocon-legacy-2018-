...
uint16_t USART_RX_STA=0;       //½ÓÊÕ×´Ì¬±ê¼Ç	
...
#define Ki 0
#define Kd 0 
//int send_num(int* bef,int* diff,CanRxMsg* RxMessage,int des){
//	         short now;
//	         int d;
//           int send;
//	         now=RxMessage->Data[2];now<<=8;now|=RxMessage->Data[3];
//					 *diff+=(int)now;
//	         d=(int)now-*bef;
//	         *bef=(int)now;
...
//					// send=-(Kp*now+Kd*d);
//					// else
//	         send=-(Kp*(now-des)+Ki**diff+Kd*d);
//	         
//	        // if(send<=500&&send>=-500) send=0;       
//	         send=(send>=2000? 2000:(send<=-2000? -2000:send));
//           return send;
//}
...
void USART1_IRQHandler(void)                	//´®¿Ú1ÖÐ¶Ï·þÎñ³ÌÐò
{
	 ...
	 u8 Res;
	 short now;
	 int send1=0;int diff1=0;int bef1=0;
	 int send2=0;int diff2=0;int bef2=0;
	 int send3=0;int diff3=0;int bef3=0;
   int time=1000000;
	 int haha=1000;
	CanRxMsg RxMessage;	
